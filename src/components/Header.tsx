...
export default function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 bg-white/95 backdrop-blur-sm border-b border-portal-light">
        ...
        <div className="flex items-center justify-between h-16 sm:h-20">
          ...
          <div className="flex items-center gap-3">
            <a
              ...
              className="hidden sm:flex items-center gap-2 text-sm font-semibold text-portal-accent no-underline hover:text-portal-accent-dark transition-colors"
            >
              <Phone size={16} />
              {BUSINESS.phone}
            </a>
            <Link
              to="/contact"
              className="hidden md:inline-flex px-4 py-2 bg-portal-accent text-white text-sm font-semibold rounded-lg no-underline hover:bg-portal-accent-dark transition-colors"
            >
              Free Estimate
            </Link>
            <button
              onClick={() => setMobileOpen(!mobileOpen)}
              className="md:hidden p-2 text-portal-gray bg-transparent border-none cursor-pointer"
      ...
      {/* Mobile Menu */}
      {mobileOpen && (
        <div className="md:hidden bg-white border-t border-portal-light">
